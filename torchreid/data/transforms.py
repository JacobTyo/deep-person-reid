...
class RandomSpeckles(object):
    ...
    def __init__(
        ...
        self.patchpool = deque(maxlen=pool_capacity)
        self.min_sample_size = min_sample_size
        self.max_num_pathces = max_num_pathces
        assert paste_type in ['random', 'all', 'color', 'patch'], 'paste_type must be random, all, color, or patch'
    ...
    def __call__(self, img):
        ...
        if self.paste_type and (self.paste_type == 'all' or self.paste_type == 'patch'):
            if len(self.patchpool) < self.min_sample_size:
                return img
        ...
        # randomly select a number of patches from the patch pool and paste them on the image
        num_patches = random.randint(max(min(10, self.max_num_pathces - 10), 0), self.max_num_pathces)
        for _ in range(num_patches):

            patchW, patchH = self.generate_wh(W, H)

            # where to put the patch?
            x1 = random.randint(0, W - patchW)
            y1 = random.randint(0, H - patchH)
            if self.paste_type == 'all':
                paste_type = random.choice(['random', 'color', 'patch'])
            else:
            ...
            elif paste_type == 'random':
                # instead of using patches from another iamge, just generate a patch of random noise
                np_patch = np.random.randint(0, 255, (patchH, patchW, 3))
                np_patch = np_patch.astype('uint8')
                patch = Image.fromarray(np_patch, 'RGB')

            elif paste_type == 'color':
                # instead of jusing patches fro another image or of noise, just generate a patch of a single random color
                norm_mean = [0.485, 0.456, 0.406]
                patch = Image.fromarray((np.ones((patchH, patchW, 3)) * norm_mean).astype('uint8'), 'RGB')
    ...
    **kwargs
    ...
    if 'random_speckles' in transforms:
        ...
        transform_tr += [RandomSpeckles(prob_happen=1.0,
                                        ...
                                        min_sample_size=10,
                                        max_num_pathces=75,
                                        patch_min_area=0.0005,
                                        patch_max_area=0.005,
                                        patch_min_ratio=0.5,
                                        prob_rotate=0.5,
                                        prob_flip_leftright=0.5,
                                        paste_type='color')]
